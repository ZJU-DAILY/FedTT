...
from traffic_model.pdformer_model import loss
from traffic_model.pdformer_model.abstract_traffic_state_model import AbstractTrafficStateModel
...
class DataEmbedding(nn.Module):
    ...
    def forward(self, x, lap_mx):
        ...
        if lap_mx.shape[1] == self.lape_dim:
...
class PDFormer(AbstractTrafficStateModel):
    def __init__(self, config, data_feature, pre_len):
        ...
        self.output_window = config.get('output_window', pre_len)#步长
        ...
        # self.device = torch.device(config.get('device', torch.device('cpu')))
        self.device = "cuda:0"
        ...
        if self.type_short_path == "dist":
            ...
            self.geo_mask[torch.tensor(sh_mx >= self.far_mask_delta)] = 1
        ...
        # print("self.lape_dim: ", self.lape_dim)
        # print("self.adj_mx: ", self.adj_mx)
        # print("self.lap_mx: ", self.lap_mx)
    ...
    def forward(self, batch, lap_mx=None):
        ...
        # x = batch['X']
        x = batch.float()
        lap_mx = self.get_lap_mx()
        # print("lap_mx: ", lap_mx)
        # print(x.dtype)
        T = x.shape[1]#torch.Size([32, 12, 170])
        x_pattern_list = []
