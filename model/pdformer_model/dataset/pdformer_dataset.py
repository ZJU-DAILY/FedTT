...
from traffic_model.pdformer_model.dataset import TrafficStatePointDataset
...
class PDFormerDataset(TrafficStatePointDataset):
    ...
    def __init__(self, config, geo_ids, adj, id=-1):
        self.id = str(id)
    ...
    def _get_dtw(self):
        cache_path = 'Dataset/new_dataset/' + self.dataset + '/pdformer_cache/dtw_' + self.dataset + '_' + self.id + '.npy'
        ...
        if not os.path.exists(cache_path) or len(self.geo_ids) != np.load(cache_path).shape[0]:
            data_mean = np.mean(
    ...
    def _load_rel(self):
        ...
        #     np.save(f'data/{self.dataset}/{self.dataset}.npy', self.sh_mx)
    ...
    def get_data(self):
        ...
        # x_train = x_train[:int(x_train.shape[0] * 0.1), :, :, :]
        # y_train = y_train[:int((y_train.shape[0]) * 0.1), :, :, :]
        # x_val = x_val[:int((x_val.shape[0]) * 0.1), :, :, :]
        # y_val = y_val[:int((y_val.shape[0]) * 0.1), :, :, :]
        ...
        self.pattern_key_file = os.path.join(
            f'Dataset/new_dataset/{self.dataset}/pdformer_cache/','pattern_keys_{}_{}_{}_{}_{}_{}'.format(
        ...
        print(4)
        return x_train.astype(np.float32), y_train.astype(np.float32), x_val.astype(np.float32), \
            y_val.astype(np.float32), x_test.astype(np.float32), y_test.astype(np.float32)

    def get_data_feature(self):
        print("self.sh_mx: ", self.sh_mx)
        print("self.sd_mx: ", self.sd_mx)
