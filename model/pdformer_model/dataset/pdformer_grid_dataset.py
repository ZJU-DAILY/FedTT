...
from traffic_model.pdformer_model.dataset import TrafficStateGridDataset
...
class PDFormerGridDataset(TrafficStateGridDataset):
    ...
    def __init__(self, config):
        ...
        # self.cache_file_name = os.path.join('federatedscope/contrib/data/pdformer_model/pdformer_cache/dataset_cache/',
        #                                     'pdformer_grid_based_{}.npz'.format(self.parameters_str))
        self.cache_file_name = config.get('file_path') + config.get('data_file')
    ...
    def _get_dtw(self):
        cache_path = f'data/{self.dataset}/pdformer_cache/dtw_' + self.dataset + '.npy'
    ...
    def get_data(self):
        ...
        self.eval_data = list(zip(x_val, y_val))
        self.test_data = list(zip(x_test, y_test))
        print("=======================")
        print(len(self.train_data), len(self.train_data[0]), len(self.train_data[0][0]), len(self.train_data[0][0][0]),
              len(self.train_data[0][0][0][0]))
        print(len(self.eval_data), len(self.eval_data[0]), len(self.eval_data[0][0]), len(self.eval_data[0][0][0]),
              len(self.eval_data[0][0][0][0]))
        print(len(self.test_data), len(self.test_data[0]), len(self.test_data[0][0]), len(self.test_data[0][0][0]),
              len(self.test_data[0][0][0][0]))
        self.train_dataloader, self.eval_dataloader, self.test_dataloader = generate_dataloader(
            self.train_data, self.eval_data, self.test_data, self.feature_name, self.batch_size, self.num_workers,
            pad_with_last_sample=self.pad_with_last_sample, distributed=self.distributed)
        self.num_batches = len(self.train_dataloader)
        self.pattern_key_file = os.path.join(
            f'data/{self.dataset}/pdformer_cache/','pattern_keys_{}_{}_{}_{}_{}_{}'.format(
        ...
        if not os.path.exists(self.pattern_key_file + ".npy"):
            ...
            if self.cluster_method == "kshape":
                ...
                for i in range(self.output_dim):
                    km = KShape(n_clusters=self.n_cluster, max_iter=self.cluster_max_iter).fit(
                        pattern_cand_keys[..., i: i + 1])
                    pattern_key_list.append(km.cluster_centers_)
                self.pattern_keys = np.concatenate(pattern_key_list, axis=-1)
            else:
                km = TimeSeriesKMeans(n_clusters=self.n_cluster, metric="euclidean",
    ...
    def __len__(self):
        return len(self.train_data) + len(self.eval_data) + len(self.test_data)

    def __getitem__(self, idx):
        if idx < len(self.train_data):
            return self.train_data[idx]
        elif idx < len(self.train_data) + len(self.eval_data):
            return self.eval_data[idx - len(self.train_data)]
        else:
            return self.test_data[idx - len(self.train_data) - len(self.eval_data)]
