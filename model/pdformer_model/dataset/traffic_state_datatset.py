...
from traffic_model.pdformer_model.dataset import AbstractDataset
from traffic_model.pdformer_model.utils import generate_dataloader
from traffic_model.pdformer_model.pdformer_utils import StandardScaler, NormalScaler, NoneScaler, \
...
class TrafficStateDataset(AbstractDataset):
    ...
    def __init__(self, config):
        ...
        self.output_window = self.config.get('output_window', 3)
        ...
        self.cache_file_name = os.path.join(f'Dataset/new_dataset/{self.dataset}/pdformer_cache/',
                                            'traffic_state_{}.npz'.format(self.parameters_str))
        self.cache_file_folder = f'Dataset/new_dataset/{self.dataset}/pdformer_cache/'
        ...
        self.geo_file = self.config.get('geo_file', '')
        self.dyna_file = self.config.get('dyna_file', '')
        self.rel_file = self.config.get('rel_file', '')
        ...
        self._load_geo()
        if os.path.exists(self.file_path+self.rel_file):
            self._load_rel()
        else:
    ...
    def _load_geo(self):
        ...
        self._logger.info("Loaded file " + self.file_path+self.geo_file + '.geo' + ', num_nodes=' + str(len(self.geo_ids)))

    def _load_grid_geo(self):
        # geofile = pd.read_csv(self.data_path + self.geo_file + '.geo')
        geofile = pd.read_csv(self.file_path+self.geo_file)
        self.geo_ids = list(geofile['geo_id'])
        self.num_nodes = len(self.geo_ids)
        self.geo_to_ind = {}
        self.geo_to_rc = {}
        ...
        self._logger.info("Loaded file " + self.file_path+self.geo_file + '.geo' + ', num_grids=' + str(len(self.geo_ids))
                          + ', grid_size=' + str((self.len_row, self.len_column)))

    def _load_rel(self):
        # relfile = pd.read_csv(self.data_path + self.rel_file + '.rel')
        relfile = pd.read_csv(self.file_path+self.rel_file)
        ...
        self._logger.info("Loaded file " + self.file_path+self.rel_file + '.rel, shape=' + str(self.adj_mx.shape))
    ...
    def _load_dyna_3d(self, filename):
        ...
        print("Loading file " + filename  + '.dyna')
        # dynafile = pd.read_csv(self.data_path + filename + '.dyna')
        dynafile = pd.read_csv(self.file_path+self.dyna_file)
        if self.data_col != '':
        ...
        self.timesolts = list(dynafile['time'][:int(dynafile.shape[0] / dynafile['entity_id'].unique().size)])
        ...
        data = []
        for i in range(self.geo_ids[0], self.geo_ids[-1]+1):
            data.append(df[i * len_time:(i + 1) * len_time].values)
    ...
    def _load_ext(self):
        extfile = pd.read_csv(self.file_path+self.ext_file)
        if self.ext_col != '':
        ...
        self._logger.info("Loaded file " + self.file_path+self.ext_file + '.ext' + ', shape=' + str(df.shape))
        return df
    ...
    def _generate_data(self):
        print("self.data_files: ", self.data_files[:1])
        ...
        print("x_list[0].shape: ", x_list[0].shape)
    ...
    def _generate_train_val_test(self):
        ...
        print("_generate_data之后的x和y: ", x.shape, y.shape, x[:1], y[:1])
